...
public class DataSeeder {
    ...
    public void seed(int total) throws Exception{
        System.out.printf("Seeding %,d records across %d threads...%n",total,THREAD_COUNT);
        ...
        for(int t=0;t<THREAD_COUNT;t++){
            ...
            executor.submit(()->{
                try{
                    ...
                    logger.log(Level.SEVERE,"Error in thread " + threadId,e);
                }finally {
                    latch.countDown();
                }
        ...
        System.out.printf("Done! %,d records seeded in %.2f seconds%n",total,elapsed/1000.0);
    }

    private void insertBatch(int count,int threadId) throws Exception{
        Faker faker=new Faker(Locale.of("en","GH"));
        ...
        try(Connection conn=dataSource.getConnection()){
            ...
            try(PreparedStatement ps=conn.prepareStatement(sql)){
                for(int i=0;i<count;i++){
                    ...
                    String email=(firstName + "."+ lastName + i + "@" + faker.internet().domainName()).toLowerCase().replaceAll("\\s+","");

                    ps.setString(1,firstName);
                    ps.setString(2,lastName);
                    ps.setString(3,email);
                    ps.setString(4,faker.phoneNumber().phoneNumber());
                    ps.setString(5,faker.address().city());
                    ps.setString(6,faker.address().country());
                    ps.setInt(7,faker.number().numberBetween(18,80));
                    ps.setString(8,faker.options().option("Male","Female"));
                    ps.addBatch();
                    ...
                    if((i+1)% BATCH_SIZE==0){
                        ps.executeBatch();
                        conn.commit();
                        System.out.printf("[Thread-%d] %,d / %,d inserted%n",threadId,i+1,count);
                    }
                }
                ps.executeBatch();
                conn.commit();
